	...
	for(int i=0; i<height; i++)
	{
			if(i%2==0)
			{
				...
				for(int j=0; j<width; j++)
				{
					...
					if(j+1<width)
					{
						newimage[( (i)*width )+ (j+1)] += (e*7/16.0);
					}
					if(i+1<height && j > 0)
					{
					 	newimage[( (i+1)*width )+ (j-1)] += (e*3/16.0);
					 	newimage[( (i+1)*width )+ (j)] += (e*5/16.0);
					}
					if(i+1<height && j+1<width)
					{
						//cout<<"#"<<i<<":"<<j<<"\t i+1 : j+1"<<endl;
						newimage[( (i+1)*width )+ (j+1)] += (e*1/16.0);
					}
					/*
					newimage_FS[( (i)*width )+ (j+1)] = newimage_FS[( (i)*width )+ (j+1)] + (unsigned char)(round)(e*7/16.0);
					newimage_FS[( (i+1)*width )+ (j-1)] = newimage_FS[( (i+1)*width )+ (j-1)] + (unsigned char)(round)(e*3/16.0);
					newimage_FS[( (i+1)*width )+ (j)] = newimage_FS[( (i+1)*width )+ (j)] + (unsigned char)(round)(e*5/16.0);
					newimage_FS[( (i+1)*width )+ (j+1)] = newimage_FS[( (i+1)*width )+ (j+1)] + (unsigned char)(round)(e*1/16.0);
					*/
